...
if [ ! -f .env ]; then
    ...
    echo "Please copy sample.env to .env and configure your API key."
    echo ""
...
if [ -z "$QUERY_REFINEMENT_LLM_API_KEY" ] || [ "$QUERY_REFINEMENT_LLM_API_KEY" = "your-api-key-here" ]; then
    ...
    echo "Please edit .env and set QUERY_REFINEMENT_LLM_API_KEY"
    echo ""
    read -p "Press Enter to exit..."
    exit 1
fi
...
if [ -z "$REFINEMENT_FRAMEWORK_PATH" ]; then
    export REFINEMENT_FRAMEWORK_PATH="./sample_framework.yaml"
fi
...
echo ""
echo "Usage:"
echo "  --framework FRAMEWORK_NAME    Use built-in framework (pico_advanced, frameworks)"
