...
Generated on 2025-11-12. This document aggregates the prompt templates embedded in the repository so stakeholders can review the language supplied to the LLM.

## Python Prompt Templates

### UNIVERSAL_FOLLOWUP_PROMPT (query_refinement_module/prompt/followup_prompt.py)
...
### UNIVERSAL_FOLLOWUP_PROMPT_CONCISE (query_refinement_module/prompt/followup_prompt.py)

```text
Evaluate if this answer is sufficient for query refinement.
...
#### Follow-up Answer Evaluation — system_prompt

```text
You evaluate whether the latest follow-up answer resolves the active refinement aspect.
Use the supplied context block to understand the original query, the running conversation, and the newest reply.
Confirm completeness, flag inconsistencies, and propose at most one additional question when more detail is required.
```