...
Awaiting: User Context Adaptation Profile, Completed Dimensions, Dependencies, Dimension Evaluation Criteria, Conversation History (if any), and Latest User Message.
""" 
...
DIMENSIONS_COMPLETED_AND_DEPENDENCIES_TEMPLATE = """
## Completed Dimensions
...
{% endfor %}
{% else %}
**No dimensions specifications completed yet**
{% endif %}
...
**Critical:** 
- **Do not re-ask about completed dimensions** (unless conflict arises). Use for consistency checking and to avoid redundancy only.
...
### Currently Evaluated Dimension Definition
...
{% for field_name, field_type in schema.items() %}
  "{{ field_name }}": <{{ field_type }}>{{ "," if not loop.last else "" }}
{% endfor %}
}
```
#TODO: JESUS -> add output

**Validation rules:**
- If `is_complete: true` → `aspect_value` must be non-null, `next_question` must be null
- If `is_complete: false` → `next_question` must be non-null, `aspect_value` can be null or partial

**Field descriptions:**
{% for field_name, field_type in schema.items() %}
- **{{ field_name }}** ({{ field_type }}): {{ descriptions.get(field_name, "No description") }}
{% endfor %}
