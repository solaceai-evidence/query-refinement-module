...
def _load_framework_aspect_ids() -> Tuple[Dict[Tuple[str, str], str], Dict[str, str]]:
    frameworks_path = Path(__file__).resolve().parents[4] / 'refinement_frameworks' / 'frameworks.yaml'
    with frameworks_path.open('r', encoding='utf-8') as handle:
        frameworks = yaml.safe_load(handle) or {}
    ...
    for framework_name, entries in frameworks.items():
        for entry in entries or []:
            ...
            by_framework_and_name[(framework_name, aspect_name)] = aspect_id
