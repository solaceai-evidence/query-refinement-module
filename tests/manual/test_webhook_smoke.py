...
def main():
    ...
    event_types = response.json().get("event_types", []) if response.status_code == 200 else []
    success = response.status_code == 200 and len(event_types) == 10
    print_result(success, f"Got {len(event_types)} event types")
    if success:
        print(f"   Events: {', '.join(event_types[:3])}...")
    ...
    if webhook_id:
        ...
        print_result(success, "Retrieved webhook details")
        if success:
