...
3. initial_summary is prioritized over final_response in dependencies
...
def test_follow_up_prompt_includes_conversation_history():
    ...
    # Most recent should be highlighted
...
def test_dependency_context_prioritizes_initial_summary():
    """Verify dependency context prioritizes initial_summary over final_response."""
    ...
    # Simulate follow-up completion with synthesis in initial_summary
    ...
    dep_step.initial_summary = "Studies published 2020-2025"  # Synthesized value
    dep_step.is_complete = True
    ...
    # Should use initial_summary (synthesized), not final_response (raw last answer)
...
def test_dependency_context_fallback_chain():
    """Verify dependency context fallback priority: initial_summary > final_response > history > clear > skipped."""
    session = QueryRefinementSession(original_query="original")
    
    # Test Priority 1: initial_summary
    aspect1 = make_aspect(aspect_id="a1", name="A1", description="D1")
    step1 = session.add_step(aspect1)
    step1.initial_summary = "Synthesized value"
    step1.is_complete = True
    
    # Test Priority 2: final_response (when no initial_summary)
...
def test_synthesis_uses_initial_summary_when_available():
    """Verify synthesis uses initial_summary (synthesized value) over raw responses."""
    ...
    # Step with initial_summary (synthesized)
    step1 = session.add_step(aspect1)
    step1.add_follow_up("Q1", "Well, I think adults")
    step1.add_follow_up("Q2", "Maybe 18-65")
    step1.initial_summary = "Adults aged 18-65"  # Clean synthesized value
    step1.is_complete = True
    
    # Step without follow-ups but with initial_summary (aspect was clear)
    step2 = session.add_step(aspect2)
    step2.initial_summary = "Already clear in query"
    step2.is_complete = True
    ...
    # Should use initial_summary for step1
    assert ("Aspect 1", "Adults aged 18-65") in clarifications
    # Should use initial_summary for step2
...
def test_end_to_end_dependency_with_synthesis():
    ...
    pop_step.initial_summary = "Adults aged 18-65 with Type 2 diabetes (excluding gestational diabetes)"
...
def test_multiple_dependencies_with_descriptions():
    ...
    pop_step.initial_summary = "Adults"
    pop_step.is_complete = True
    
    time_step = session.add_step(time)
    time_step.initial_summary = "2020-2025"
